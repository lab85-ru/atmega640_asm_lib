...
;	reg = reg ^ table_crc32[data];
				ldi		zl,low(table_crc32)				; БЕЗ множения на 2 тк отрабатываем переход за предел 64 К		
				ldi		zh,high(table_crc32)

				rol		zh
;				brcs	_crc32ram_rampz1
;				clr 	r16
;				out		RAMPZ,r16
;				rjmp	_crc32ram_rampz_ex
;_crc32ram_rampz1:
;				ldi 	r16,(1<<RAMPZ0)
;				out		RAMPZ,r16
;_crc32ram_rampz_ex:
...
;умноженное на 4 тк дынне 4 байта
				clr		data1

				rol		data0
				brcc	_crc32ram_add_0
...
;	reg = reg ^ table_crc32[data];
		ldi		zl,low(table_crc32*2)		
		ldi		zh,high(table_crc32*2)
...
/*
unsigned long int reg, data, i;
unsigned char ddd[]="123456789";
	reg = 0xffffffff;
